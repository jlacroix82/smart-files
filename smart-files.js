...
function watchDirectory(dir, interval = 30, opts = {}) {
  const { dryRun = false, rules = null } = opts;
  ...
  const rulesHint = (rulesFile || '').length > 0 && fs.existsSync(rulesFile) ? ` rules from .smart-files-rules.json` : ` (default rules)`;
  const modeHint = dryRun ? ' [DRY RUN — no files moved]' : '';
  console.log(`[smart-files] Watching: ${watchDir}${rulesHint}${modeHint}`);
  ...
  function processFile(filepath) {
    // Skip if file no longer exists (moved by prior event)
    if (!fs.existsSync(filepath)) return;
    const stat = fs.statSync(filepath);
    // Skip directories, hidden files, and our own config files
    ...
    if (hash && baselineHashes[hash]) {
      console.log(`  ⏭️  Skip (dup of ${path.relative(watchDir, baselineHashes[hash])}): ${name}`);
      journalAction('skip-dup', filepath, { reason: `duplicate of ${path.relative(watchDir, baselineHashes[hash])}` });
      return;
    }
    ...
    if (cleanName !== name) {
      ...
      if (!dryRun && cleanName !== name && !fs.existsSync(renamed)) {
    ...
    if (fs.existsSync(destPath)) {
      ...
      const extPart = cleanName.replace(/^[^.]+\./, '');
      const collisionName = `${base}-${ts}.${extPart}`;
      cleanName = collisionName;
      destPath = path.join(destDir, collisionName);
    }
...
switch (mode) {
  ...
  default:
    ...
    console.log('  --watch <dir> [interval]  → Auto-organize files (fs.watch + polling)');
    console.log('  --dry-run                 → Preview without moving files (for --watch)');
  ...
  case 'watch': {
    if (!watchDir) {
      ...
      watchDirectory(watchDir, watchInterval, { dryRun });
    }
    break;
  }
}
